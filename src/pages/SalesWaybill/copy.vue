          ...
          <b-row>
           <NextFormGroup item-key="InvoiceNumber" :error="$v.form.InvoiceNumber" md="2" lg="2">
            ...
            <NextFormGroup item-key="RouteId" :error="$v.form.RouteId" md="2" lg="2">
              <NextDropdown v-model="selectedRoute" @input="selectedSearchType('RouteId', $event)" label="Description1" url="VisionNextRoute/api/Route/AutoCompleteSearch" searchable />
            </NextFormGroup>
...
export default {
  ...
  computed: {
    ...mapState(['representatives', 'createCode', 'customers', 'priceList', 'vehicles', 'paymentPeriods', 'currencies', 'orderStatusList', 'items', 'priceListItems', 'stocks', 'eDocumentStatus', 'invoiceKinds'])
  },
  mounted () {
  ...
  methods: {
    ...
    setData () {
      ...
      if (rowData) {
        ...
        this.$store.dispatch('getCreateCode', {...this.query, apiUrl: 'VisionNextInvoice/api/SalesWaybill/GetCode'}).then(() => {
          if (this.createCode) {
            this.form.InvoiceNumber = this.createCode
          }
        })
    ...
    save () {
      ...
      if (this.$v.form.$error) {
        ...
        this.form.RecordId = this.$route.params.url
        let model = {
          'model': this.form
        }
        this.$api.postByUrl(model, 'VisionNextInvoice/api/SalesWaybill/CopyDispatchSaveAs').then((res) => {
