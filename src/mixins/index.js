...
export default {
  ...
  methods: {
    ...
    onlyForCurrencyDot ($event) {
      let model = $event.currentTarget.value
      let keyCode = ($event.keyCode ? $event.keyCode : $event.which)

      if ((keyCode < 48 || keyCode > 57) && ((keyCode !== 46 && keyCode !== 44) || model.indexOf('.') !== -1 || model.indexOf(',') !== -1)) {
        $event.preventDefault()
      }
      if ((model !== null && model.indexOf('.') > -1 && (model.split('.')[1].length > 0)) || (model !== null && model.indexOf(',') > -1 && (model.split(',')[1].length > 0))) {
        $event.preventDefault()
      }
    },
    onlyForCurrencyDotOrComma ($event) {
      let model = $event.currentTarget.value
      let keyCode = ($event.keyCode ? $event.keyCode : $event.which)

      if ((keyCode < 48 || keyCode > 57) && ((keyCode !== 44 && keyCode !== 46) || (keyCode === 188) || model.indexOf('.') !== -1 || model.indexOf(',') !== -1)) {
        $event.preventDefault()
      }
