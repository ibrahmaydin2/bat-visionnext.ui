          ...
          <b-row>
           <NextFormGroup item-key="InvoiceNumber" :error="$v.form.InvoiceNumber" md="2" lg="2">
            ...
            <NextFormGroup item-key="InvoiceTypeId" :error="$v.form.InvoiceTypeId" md="2" lg="2">
              <v-select v-model="selectedInvoiceType" label="InvoiceTypeId" :options="invoiceTypes" :filterable="false" @input="selectedSearchType('InvoiceTypeId', $event)" :disabled="true"></v-select>
            </NextFormGroup>
          </b-row>
          <b-row>
...
export default {
  ...
  computed: {
    ...mapState(['representatives', 'customers', 'currencies', 'items', 'invoiceTypes', 'discountReasons', 'invoiceKinds'])
  },
  mounted () {
