<template>
  <div>
    <b-dropdown-group v-if="workFlowList.length > 0" :header="$t('header.workFlow')">
      <b-dropdown-item @click="getWorkFlowProcessModel(workFlow.RecordId, workFlow.callback)" v-for="(workFlow, i) in workFlowList" :key="i">
...
export default {
  ...
  methods: {
    getAllData () {
      if (!this.value || !this.value.ClassName || !this.value.PageName) {
        return
      }
      let request = {
    ...
    getWorkFlowProcessModel (workflowId, callback) {
      if (callback) {
        callback()
        return
      }
