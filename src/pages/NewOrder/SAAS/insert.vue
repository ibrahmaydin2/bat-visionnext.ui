<template>
  <b-row>
          ...
          <b-col md="12" lg="9" sm="12">
            <b-row>
              ...
              <NextFormGroup item-key="DocumentDate" :error="$v.form.DocumentDate" md="3" lg="3">
                <NextDatePicker v-model="form.DocumentDate" :disabled="insertReadonly.DocumentDate" @input="selectDocumentDate" />
              </NextFormGroup>
              ...
              <NextFormGroup item-key="RepresentativeId" :error="$v.form.RepresentativeId" md="3" lg="3">
                  <NextDropdown :disabled="insertReadonly.RepresentativeId" @input="selectedSearchType('RepresentativeId', $event)" orConditionFields="Code,Description1,Name,Surname" url="VisionNextEmployee/api/Employee/AutoCompleteSearch" searchable />
                </NextFormGroup>
    ...
    <b-col cols="12">
      <b-tabs>
        <b-tab :title="$t('insert.newOrder.products')" @click.prevent="tabValidation()" v-if="form.CustomerId && form.DocumentDate">
          <b-row>
...
export default {
  ...
  methods: {
    getInsertPage (e) {
      ...
      this.form.DocumentDate = currentDate.toISOString().slice(0, 10)
    ...
    searchPriceListItem () {
      if (!this.selectedPrice || !this.selectedPrice.RecordId || !this.orderLine.item) {
        return false
      }
      let model = {
        andConditionModel: {
      ...
      if (value) {
        ...
        this.$api.postByUrl(request, 'VisionNextItem/api/Item/Search').then((response) => {
          if (response && response.ListModel && response.ListModel.BaseModels.length > 0) {
            if (index > 0) {
              this[`category${index}List`] = response.ListModel.BaseModels.map(r => {
                return {
                  Label: r[`Category${index}Id`],
                  DecimalValue: r[`Category${index}Id`]
                }
              })
            } else {
