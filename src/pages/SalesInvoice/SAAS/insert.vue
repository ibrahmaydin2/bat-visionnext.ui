        ...
        <b-tab :title="$t('insert.order.enterInvoice')" active @click.prevent="tabValidation()">
          <b-row>
            ...
            <NextFormGroup item-key="RepresentativeId" :error="$v.form.RepresentativeId" md="2" lg="2">
              <NextDropdown @input="selectedSearchType('RepresentativeId', $event)" label="Description1" orConditionFields="Code,Description1,Name,Surname" url="VisionNextEmployee/api/Employee/AutoCompleteSearch" searchable :disabled="insertReadonly.RepresentativeId"/>
            </NextFormGroup>
            ...
            <NextFormGroup item-key="PaymentTypeId" :error="$v.form.PaymentTypeId" md="2" lg="2">
              <NextDropdown
                ...
                :source="paymentTypes"
                label="Label"
                @input="selectedType('PaymentTypeId', $event); setIsCashCollection($event)"
  ...
  methods: {
    ...
    getPaymentTypes () {
      ...
      this.$api.post({RecordId: this.form.CustomerId}, 'Customer', 'Customer/Get').then((res) => {
        ...
        this.setIsCashCollection(me.selectedPaymentType)
      })
    },
    save () {
      this.$v.form.$touch()
