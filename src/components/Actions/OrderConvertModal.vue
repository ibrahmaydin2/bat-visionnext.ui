<template>
  <b-modal v-if="modalAction" id="orderConvertModal" :title="modalAction.Title" size="xl" no-close-on-backdrop>
    <section>
      <b-row>
        <NextFormGroup :title="$t('index.OrderConvert.Code')" md="4" lg="4">
          <b-form-input type="text" v-model="form.Code" readonly />
        </NextFormGroup>
        <NextFormGroup :title="$t('index.OrderConvert.Warehouse')" md="4" lg="4">
          <b-form-input type="text" v-model="warehouse" readonly />
        </NextFormGroup>
        <NextFormGroup :title="$t('index.OrderConvert.InvoiceKindId')" :error="$v.form.InvoiceKindId" :required="false" md="4" lg="4">
          <v-select v-model="invoiceType" :options="invoiceTypes" label="label" @input="selectedType('InvoiceKindId', $event)"></v-select>
        </NextFormGroup>
        <NextFormGroup :title="$t('index.OrderConvert.DocumentNumber')" md="4" lg="4">
          <b-form-input type="text" v-model="form.DocumentNumber" />
        </NextFormGroup>
        <NextFormGroup :title="$t('index.OrderConvert.Employee')" md="4" lg="4">
        ...
        <NextFormGroup :title="$t('index.OrderConvert.TransactionDate')" md="4" lg="4">
  ...
  data () {
    return {
      ...
      fields: [
        {
          key: 'ItemCode',
          label: this.$t('index.OrderConvert.ItemCode'),
          sortable: true,
        ...
        {
          key: 'Item',
          label: this.$t('index.OrderConvert.Item'),
          sortable: true,
          formatter: (value, key, item) => {
        ...
        {
          key: 'Quantity',
          label: this.$t('index.OrderConvert.Quantity'),
          sortable: true
        },
        {
          key: 'Stock',
        ...
        {
          key: 'Count',
          label: this.$t('index.OrderConvert.Count'),
          sortable: true
        }
      ]
    }
  ...
  methods: {
    getCustomer () {
      this.$api.postByUrl({RecordId: this.modalItem.CustomerId}, 'VisionNextCustomer/api/Customer/Get').then((res) => {
        if (res.Model) {
          this.invoiceTypes = this.getOrderDocumentTypes(res.Model.SalesDocumentTypeId)
        }
      })
    },
    getCode () {
